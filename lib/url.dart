// const url = 'https://7148-111-119-49-209.in.ngrok.io';
// const url = 'http://192.168.100.172:8000';
// const url = 'http://192.168.101.165:8000';

//pp number = 699-526-518-7
//mob number =9851228881

// const url = 'http://192.168.100.54:8000';
// const wsUrl = 'ws://192.168.100.54:8000';
// const url = 'http://192.168.1.70:8000';
// const wsUrl = 'ws://192.168.1.70:8000';
const String url = 'http://65.109.161.97:8000';
const String wsUrl = 'ws://65.109.161.97:8000';
const String getDataUrl = '$url/api/v1/identity/documents/';
const String postMobileAndNinUrl = '$url/api/v1/auth/';
const String postOtp = '$url/api/v1/auth/verify/';
const String getDataQrUrl = '$url/api/v1/qr/request';
const String getPermitIdUrl = '$wsUrl/ws/qr/permit';
const String getPidDataUrl = '$url/api/v1/qr/permit';
const String getHistoryUrl = '$url/api/v1/history/';
const String scanRequestApprovalUrl = '$url/api/v1/scan-request';
